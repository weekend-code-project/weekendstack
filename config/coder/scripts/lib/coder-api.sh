...
prompt_for_coder_setup() {
    ...
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo ""
    log_info "Coder is now running at: $CODER_URL"
    echo ""
    echo "Please complete these steps to enable template deployment:"
    echo ""
    echo "  1. Open Coder in your browser:  $CODER_URL"
    echo "  2. Create your admin account (first user becomes admin)"
    echo "  3. Log in to Coder"
    echo "  4. Click your avatar → Settings → Tokens"
    echo "  5. Click 'Create Token'"
    echo "  6. Give it a name (e.g., 'WeekendStack Setup')"
    echo "  7. Set expiration (recommend: 30 days or longer)"
    echo "  8. Copy the generated token"
    echo ""
    read -p "Press ENTER when you have completed steps 1-8... "
    echo ""
