...
module "agent" {
  ...
  startup_script = join("\n", [
    ...
    "echo '[DEBUG] Workspace password provided: ${data.coder_parameter.workspace_secret.value != "" ? "YES" : "NO"}'",
    data.coder_parameter.workspace_secret.value != "" ? local.traefik_auth_setup_script : "",
