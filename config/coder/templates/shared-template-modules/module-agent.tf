# (Shared central) module-agent.tf
module "agent" {
  ...
  os   = "linux"
  depends_on = [ null_resource.ensure_workspace_folder ]
  startup_script = join("\n", [
    "#!/bin/bash","set -e",
  ...
  env_vars = { SSH_PORT = module.ssh.ssh_port PORTS = join(",", local.exposed_ports_list) PORT = element(local.exposed_ports_list, 0) }
