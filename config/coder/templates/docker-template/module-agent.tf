...
module "agent" {
  ...
  startup_script = join("\n", [
    ...
    data.coder_parameter.enable_docker.value ? try(module.docker[0].docker_install_script, "") : "",
    data.coder_parameter.enable_docker.value ? try(module.docker[0].docker_config_script, "") : "",
