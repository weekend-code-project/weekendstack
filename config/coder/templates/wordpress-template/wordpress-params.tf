...
# Fix permissions - Apache needs to traverse parent directories
...
# Restart Apache and verify it's running
echo '[WordPress] 🔄 Starting Apache web server...'
...
if sudo service apache2 status > /dev/null 2>&1; then
  ...
  echo '[WordPress] ❌ Apache failed to start'
