...
module "agent" {
  ...
  startup_script = join("\n", [
    ...
    "    if grep -q 'CODER_PATCH_ALLOWED_HOSTS' \"$VITE_CONFIG\" 2>/dev/null; then",
    "      echo \"[VITE] Already patched, skipping\"",
    "    else",
    "      # Only add allowedHosts if not present",
    "      if ! grep -q 'allowedHosts' \"$VITE_CONFIG\" 2>/dev/null; then",
    "        sed -i \"/server:\\s*{/a\\    // CODER_PATCH_ALLOWED_HOSTS\\n    allowedHosts: [\\\"$WORKSPACE_DOMAIN\\\"],\" \"$VITE_CONFIG\"",
    "      fi",
    "      ",
    "      # Only add hmr if not present",
    "      if ! grep -q 'hmr:' \"$VITE_CONFIG\" 2>/dev/null; then",
    "        # Find the closing brace of the server block and add hmr before it",
