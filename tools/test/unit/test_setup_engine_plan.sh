...
if [[ "$effective_profiles" == "core,ai,open-webui,paperclip,ollama-cpu,networking,external" ]] && \
...
test_case "planner reports tunnel warnings and manual followups"
...
if [[ "$warning_count" -ge 1 ]] && \
   ...
   [[ "$manual_followups" == *"paperclip"* ]]; then
    ...
    test_fail "Expected tunnel warning and manual followups, got warnings=$warning_count followups=$manual_followups"
fi
